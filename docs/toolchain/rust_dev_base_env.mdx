...
```
![](/toolchain-images/uninstall_rust.png)
...
![](/toolchain-images/visual_studio_installer_startup.png)

在弹出的窗口里确保红色箭头指示的这些组件被成功安装。

![](/toolchain-images/visual_studio_installer_desktop_dev.png)

![](/toolchain-images/visual_studio_installer_win_sdk.png)
...
```
![](/toolchain-images/rustup_env_path.png)
...
```
![](/toolchain-images/cargo_env_path.png)
...
![](/toolchain-images/rustup_init_startup.png)

随后，输入`2`，按下`Enter`。
![](/toolchain-images/rustup_init_cusomize_installation.png)
紧接着会弹出确认信息，继续按`Enter`。
![](/toolchain-images/rustup_init_cusomize_installation_host.png)
随后会弹出确认`toolchain`选择信息，此时输入`nightly`并且按下`Enter`。
![](/toolchain-images/rustup_init_cusomize_installation_toolchain.png.png)
随后还会确认`profile`，按`Enter`即可。
![](/toolchain-images/rustup_init_cusomize_installation_profile.png)
最后会询问是否要`modify`环境变量，也就是采用我们之前配置的环境变量。输入`y`并且按下`Enter`。
![](/toolchain-images/rustup_init_cusomize_installation_path.png)
按下`Enter`之后，会再次出现以下提示信息。
![](/toolchain-images/rustup_init_cusomize_installation_final.png)
此时，只需要按下`Enter`即可。
![](/toolchain-images/rustup_init_cusomize_installation_process.png)
看到终端里的`Rust is installed now. Great!`，则说明此次安装圆满完成。
![](/toolchain-images/OK.png)
